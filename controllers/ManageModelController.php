...
class ManageModelController extends YAdminController {
	...
	public function actionList ( $model_name ) {
		...
		$this->breadcrumbs = array($this->module->getModelNamePlural($model),);
		...
		$data2 = array(
			...
			'columns' => array(
				array(
					...
					'updateButtonUrl' => $url_prefix . 'update",array("model_name"=>"' . get_class(
						$model
					) . '","pk"=>$data->primaryKey))',

					'deleteButtonImageUrl' => $assetesUrl . '/img/admin/icon_deletelink.gif',
					'deleteButtonUrl' => $url_prefix . 'delete",array("model_name"=>"' . get_class(
						$model
					) . '","pk"=>$data->primaryKey))',
					'viewButtonUrl' => $url_prefix . 'view",array("model_name"=>"' . get_class(
						$model
					) . '","pk"=>$data->primaryKey))',
	...
	public function actionToggle () {
		...
		if ( $errors ) {
			Ajax::send(
				Ajax::AJAX_ERROR,
				YiiadminModule::t(
					'При сохранении возникли ошибки: {errors}',
	...
	public function actionCreate () {
		...
		$this->breadcrumbs = array(
			$this->module->getModelNamePlural($model) => $this->createUrl(
				'manageModel/list',
		...
		if ( Yii::app()->request->isPostRequest ) {
			...
			foreach ( $relationsToSave AS $relationName => $relatedModelsAry ) {
				...
				foreach ( $relatedModelsAry AS $num => $relatedModel ) {
					...
					if ( isset($_POST[$relatedModelName][$num]) && sizeof(
						array_filter($_POST[$relatedModelName][$num])
					)
					) {
	...
	protected function redirectUser ( $model_name, $pk ) {
		...
		if ( isset($_POST['_addanother']) ) {
			Yii::app()->user->setFlash(
				'flashMessage',
				YiiadminModule::t('Изменения сохранены. Можете создать новую запись.')
			);
